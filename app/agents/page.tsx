...
const AGENT_RECORD_COLUMN_COUNT = 21 + AGENT_STATUS_COLUMNS.length
          ...
          <div className="relative  overflow-x-auto">
            ...
            <tbody>
              {isUpdating ? (
                ...
                <tr>
                  <td colSpan={AGENT_RECORD_COLUMN_COUNT} className="py-10 text-center text-sm text-[#64748B] font-poppins">
                    No agents found
                  </td>
                </tr>
              ) : (
                filteredAgentsList.map((agent: any, idx: number) => {
                  ...
                  return (
                    <tr key={`${agent.name}-${idx}`} className="border-b border-white/[0.04] hover:bg-white/[0.03] transition-colors">
                      ...
                      {AGENT_STATUS_COLUMNS.map((column) => {
                        const amount = Number(agent[column.amountKey] || 0)
                        const nop = Number(agent[column.nopKey] || 0)
                        return (
