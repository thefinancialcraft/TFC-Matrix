...
export default function TeamsPage() {
  ...
  // Calculate KPI metrics for teams
  const kpiMetrics = useMemo(() => {
    const totalTeams = teamLeadersList.length
    const targetCompleted = teamLeadersList.filter(t => Number(t.percent || 0) >= 100).length
    const onZero = teamLeadersList.filter(t => Number(t.nop || 0) === 0).length
    const justificationCleared = teamLeadersList.filter(t => Number(t.percent || 0) >= 50).length
    // Performers: teams with target completed
    const performers = teamLeadersList.filter(t => Number(t.percent || 0) >= 100).length
  ...
  const filteredTeamLeadersList = useMemo(() => {
    if (!searchQuery.trim()) return teamLeadersList
    const query = searchQuery.toLowerCase()
    return teamLeadersList.filter(team => 
      team.name && team.name.toLowerCase().includes(query)
    )
  }, [teamLeadersList, searchQuery])
    ...
    const fetchTeamLeaders = async () => {
      try {
        ...
        if (data.teamLeaders && Array.isArray(data.teamLeaders)) {
          const sortedTeams = [...data.teamLeaders].sort((a, b) => Number(b.percent || 0) - Number(a.percent || 0))
          setTeamLeadersList(sortedTeams)
        } else {
        ...
        if (data.monthlyTeamData) {
          setMonthlyTeamData(data.monthlyTeamData)
        } else {
          setMonthlyTeamData({})
        }
      } catch (error) {
  ...
  const exportToCSV = () => {
    if (viewMode === 'matrix') {
      // Export matrix data
      const headers = ['Team', 'Parameter', ...selectedMonth.map(date => {
        const month = availableMonths.find(m => m.date === date)
        return month?.name || date
      })]
      
      const rows: (string | number)[][] = []
      filteredTeamLeadersList.forEach((team: any) => {
        selectedParameter.forEach((param) => {
          const row = [team.name, param]
          selectedMonth.forEach((monthDate) => {
            const teamMonthData = monthlyTeamData[monthDate]?.find((t: any) => t.name === team.name)
            ...
            const value = teamMonthData?.[field] !== undefined ? teamMonthData[field] : '-'
      ...
      link.setAttribute('download', 'teams_matrix.csv')
      ...
      // Export records data
      const headers = ['Team Name', 'NOP', 'Score', 'Ach%', 'Pay Date', 'Days', 'Salary', 'Target', 'Justification', 'Just Left', 'Gross', '50 Below', '50k+', '1L+', '1Yr NOP', 'Multi NOP', 'Disc Del', 'Disc #', 'Pay Amt', 'Total Agents', 'Zero Score', 'Target Cleared', 'Just Cleared']
      const rows = filteredTeamLeadersList.map((team: any) => [
        team.name,
        team.nop || 0,
        Number(team.score || 0),
        Number(team.percent || 0).toFixed(1) + '%',
        formatDate(team.payDate),
        team.days || 0,
        team.salary ? `₹${Number(team.salary).toLocaleString('en-IN')}` : 'N/A',
        Number(team.target || 0),
        team.justification ? `₹${Number(team.justification).toLocaleString('en-IN')}` : 'N/A',
        team.justLeft ? `₹${Number(team.justLeft).toLocaleString('en-IN')}` : 'N/A',
        Number(team.gross || 0),
        team.fiftyBelow || 0,
        team.fiftykAbove || 0,
        team.oneLacAbove || 0,
        team.singleYrNop || 0,
        team.multiYrNop || 0,
        team.discountDelivered ? `₹${Number(team.discountDelivered).toLocaleString('en-IN')}` : 'N/A',
        team.discountCount || 0,
        team.payAmt > 0 ? Number(team.payAmt) : 0,
        team.totalAgents || 0,
        team.zeroScoreAgents || 0,
        team.targetCompletedAgents || 0,
        team.justificationClearedAgents || 0
      ])
    ...
    const url = URL.createObjectURL(blob)
    link.setAttribute('href', url)
    link.setAttribute('download', 'teams_records.csv')
    link.style.visibility = 'hidden'
    document.body.appendChild(link)
    link.click()
    document.body.removeChild(link)
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 pb-8 max-w-7xl mx-auto">
      <div className="flex flex-wrap justify-between items-end gap-4 mb-6">
        <div>
          <h1 className="silver-shimmer-text font-poppins text-[24px] sm:text-[28px] font-normal tracking-normal leading-none mb-2">Teams Dashboard</h1>
        </div>
        ...
        <div className="relative h-[80px] p-4 pr-5 flex items-center justify-between rounded-br-[24px] overflow-hidden bg-transparent transition-all duration-300 group">
          ...
          <div className="flex flex-col items-end text-right">
            <span className="font-roboto text-[24px] font-bold text-white leading-none">
              {isUpdating ? '...' : kpiMetrics.totalTeams}
            </span>
                  ...
                  <div className="absolute top-full left-0 mt-2 z-[9999] bg-white rounded-xl shadow-2xl w-[240px]">
                    ...
                    <div className="relative" ref={parameterDropdownRef}>
                      ...
                      {showParameterDropdown && (
                        <div className="absolute z-[1000] w-full mt-1 bg-white border-2 border-gray-200 rounded-2xl shadow-2xl max-h-32 overflow-y-auto scrollbar-hide">
                          {[
                            'NOP', 'Score', 'Ach%', 'Pay Date', 'Days', 'Salary', 'Target',
          ...
          <div className="relative  overflow-x-auto">
            ...
            <thead>
              <tr className="border-b border-white/[0.08]">
                ...
                <th className="pb-3 pr-4 text-[13px] font-poppins font-medium text-white/50 text-center">Team Name</th>
                ...
                <th className="pb-3 pr-4 text-[13px] font-poppins font-medium text-white/50 text-center">Pay Amt</th>
                <th className="pb-3 pr-4 text-[13px] font-poppins font-medium text-white/50 text-center">Total Agents</th>
                <th className="pb-3 pr-4 text-[13px] font-poppins font-medium text-white/50 text-center">Zero Score</th>
                <th className="pb-3 text-[13px] font-poppins font-medium text-white/50 text-center">Target Cleared</th>
                <th className="pb-3 text-[13px] font-poppins font-medium text-white/50 text-center">Just Cleared</th>
              </tr>
            </thead>
            <tbody>
              {isUpdating ? (
                ...
                filteredTeamLeadersList.map((team: any, idx: number) => {
                  const initials = team.name
                    ? team.name.split(' ').map((n: string) => n[0]).filter(Boolean).slice(0, 2).join('').toUpperCase()
                    : 'TM'

                  return (
                    <tr key={`${team.name}-${idx}`} className="border-b border-white/[0.04] hover:bg-white/[0.03] transition-colors">
                      ...
                      <td className="py-3.5 pr-4 text-center">
                        <span className="font-roboto text-[13px] text-white/80">₹{Number(team.score || 0).toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span>
                      </td>
                      <td className="py-3.5 pr-4 text-center">
                        <span className={`font-roboto text-[13px] font-bold ${
                          Number(team.percent || 0) >= 100 ? 'text-emerald-400' :
                          Number(team.percent || 0) >= 50 ? 'text-yellow-400' :
                          Number(team.percent || 0) > 0 ? 'text-orange-400' : 'text-red-400'
                        }`}>
                          {Number(team.percent || 0).toFixed(1)}%
                        </span>
                      </td>
                      ...
                      <td className="py-3.5 pr-4 text-center">
                        <span className={`font-roboto text-[13px] font-bold ${
                          team.days <= 30 ? 'text-emerald-400' :
                          team.days <= 60 ? 'text-yellow-400' :
                          team.days <= 90 ? 'text-orange-400' : 'text-red-400'
                        }`}>
                          {team.days || 0}d
                        </span>
                      </td>
                      <td className="py-3.5 pr-4 text-center">
                        <span className="font-roboto text-[13px] text-white/60">{team.salary ? `₹${Number(team.salary).toLocaleString('en-IN')}` : 'N/A'}</span>
                      </td>
                      <td className="py-3.5 pr-4 text-center">
                        <span className="font-roboto text-[13px] text-white/80">₹{Number(team.target || 0).toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span>
                      </td>
                      <td className="py-3.5 pr-4 text-center">
                        <span className="font-roboto text-[13px] text-white/60">{team.justification ? `₹${Number(team.justification).toLocaleString('en-IN')}` : 'N/A'}</span>
                      </td>
                      <td className="py-3.5 pr-4 text-center">
                        <span className="font-roboto text-[13px] text-white/60">{team.justLeft ? `₹${Number(team.justLeft).toLocaleString('en-IN')}` : 'N/A'}</span>
                      </td>
                      <td className="py-3.5 pr-4 text-center">
                        <span className="font-roboto text-[13px] text-white/60">₹{Number(team.gross || 0).toLocaleString('en-IN', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</span>
                      </td>
                      <td className="py-3.5 pr-4 text-center">
                        <span className="font-roboto text-[13px] text-white/60">{team.fiftyBelow || 0}</span>
                      </td>
                      <td className="py-3.5 pr-4 text-center">
                        <span className="font-roboto text-[13px] text-white/60">{team.fiftykAbove || 0}</span>
                      </td>
                      <td className="py-3.5 pr-4 text-center">
                        <span className="font-roboto text-[13px] text-white/60">{team.oneLacAbove || 0}</span>
                      </td>
                      <td className="py-3.5 pr-4 text-center">
                        <span className="font-roboto text-[13px] text-white/80">{team.singleYrNop || 0}</span>
                      </td>
                      <td className="py-3.5 pr-4 text-center">
                        <span className="font-roboto text-[13px] text-white/80">{team.multiYrNop || 0}</span>
                      </td>
                      <td className="py-3.5 pr-4 text-center">
                        <span className="font-roboto text-[13px] text-white/60">{team.discountDelivered ? `₹${Number(team.discountDelivered).toLocaleString('en-IN')}` : 'N/A'}</span>
                      </td>
                      <td className="py-3.5 pr-4 text-center">
                        <span className="font-roboto text-[13px] text-white/80">{team.discountCount || 0}</span>
                      </td>
                      <td className="py-3.5 pr-4 text-center">
                        <span className="font-roboto text-[13px] text-white/60">{team.payAmt > 0 ? `₹${Number(team.payAmt).toLocaleString('en-IN')}` : 'N/A'}</span>
                      </td>
                      <td className="py-3.5 pr-4 text-center">
                        <span className="font-roboto text-[13px] text-white/80">{team.totalAgents || 0}</span>
                      </td>
                      <td className="py-3.5 pr-4 text-center">
                        <span className="font-roboto text-[13px] text-red-400">{team.zeroScoreAgents || 0}</span>
                      </td>
                      <td className="py-3.5 pr-4 text-center">
                        <span className="font-roboto text-[13px] text-emerald-400">{team.targetCompletedAgents || 0}</span>
                      </td>
                      <td className="py-3.5 text-center">
                        <span className="font-roboto text-[13px] text-yellow-400">{team.justificationClearedAgents || 0}</span>
                      </td>
                    </tr>
                  )
                })
                  ...
                  filteredTeamLeadersList.map((team: any, idx: number) => {
                    ...
                    return (
                      selectedParameter.map((param, paramIdx) => (
                        <tr key={`${team.name}-${param}`} className={`${paramIdx === selectedParameter.length - 1 ? 'border-b border-white/[0.04]' : ''} hover:bg-white/[0.03] transition-colors`}>
                          {paramIdx === 0 ? (
                            <>
                              <td rowSpan={selectedParameter.length} className="py-2 pr-4 align-top">
                        ...
                        {selectedMonth.length > 0 ? [...selectedMonth].reverse().map((monthDate) => {
                          const monthData = monthlyTeamData[monthDate] || []
                          const teamMonthData = monthData.find((m: any) => m.name === team.name) || {}
